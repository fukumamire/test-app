      ...
      <form method="post" action="{{route('contact.store')}}" enctype="multipart/form-data">
        ...
        <div class="md:flex items-center mt-8">
          <div class="w-full flex flex-col">
            ...
            <input type="text" name="title" class="w-auto py-2 text-xl placeholder-gray-300 border border-gray-300 rounded-md" id="title" value="{{old('title')}}" placeholder="タイトルを２５５文字以内で記載してください">
          </div>
        </div>
        <div class="w-full flex flex-col">
          <label for="body" class="font-semibold text-2xl leading-none mt-4">本文</label>
          <textarea name="body" class="w-auto py-2 text-xl placeholder-gray-300 order border-gray-300 rounded-md" id="body" cols="30" rows="10" placeholder="本文は１０００文字以内でお願います">{{old('body')}}</textarea>
        </div>

        <div class="md:flex items-center">
          <div class="w-full flex flex-col">
            <label for="email" class="font-semibold text-2xl leading-none mt-4">メールアドレス</label>
            <input type="text" name="email" class="w-auto py-2 text-xl placeholder-gray-300 border border-gray-300 rounded-md" id="email" value="{{old('email')}}" placeholder="Enter Email">
          </div>
        </div>
